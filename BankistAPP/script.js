...
const displayMovements = function (acc, sort = false) {
  ...
  const movs = sort
    ? acc.movements.slice().sort((a, b) => a.movement - b.movement)
    : acc.movements;
...
const calcDisplayBalance = function (acc) {
  acc.balance = acc.movements.reduce((acc, mov) => acc + mov.movement, 0);
...
const calcDisplaySummary = function (acc) {
  const incomes = acc.movements
    ...
    .reduce((acc, mov) => acc + mov.movement, 0);
  ...
  const interest = acc.movements
    .filter((mov) => mov.movement > 0)
    .map((deposit) => (deposit.movement * acc.interestRate) / 100)
    .filter((int, i, arr) => {
      return int >= 1;
    })
...
btnLoan.addEventListener("click", function (e) {
  ...
  if (
    amount > 0 &&
    currentAccount.movements.some((mov) => mov.movement >= amount * 0.1)
  ) {
    setTimeout(function () {
      // Add movement && Add loan date
      currentAccount.movements.push({
        movement: amount,
        date: new Date().toISOString(),
      });
